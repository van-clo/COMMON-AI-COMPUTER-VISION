        ...
        for j in range(1, 5):
            ...
            try:
                filename_predictions = (
                    predictions_path + name + "_" + str(j) + "_predicted.txt"
                )
                filename_ground_truth = (
                    ground_truth_path + name + "_" + str(j) + "_gt.txt"
                )
            ...
            if verbose:
                print(
                    "Task 1 - classifying road lanes: for test example number ",
                    str(i),
                    "image ",
                    str(j),
                    " the prediction is :",
                    (1 - correct_prediction) * "in" + "correct" + "\n",
                )
...
def compute_percentage_tracking(gt_bboxes, predicted_bboxes, num_frames, verbose=0):
    ...
    for i in range(num_frames):
        if (
            gt_dict.get(i, None) is None and pred_dict.get(i, None) is None
        ):  # the bowling bowl is not on the lane
            tn += 1

        elif (
            gt_dict.get(i, None) is not None and pred_dict.get(i, None) is None
        ):  # the bowling ball is not detected
            fn += 1

        elif (
            gt_dict.get(i, None) is None and pred_dict.get(i, None) is not None
        ):  # the bowling ball is not on the lane, but it is 'detected'
            fp += 1

        elif (
            gt_dict.get(i, None) is not None and pred_dict.get(i, None) is not None
        ):  # the bowling ball is on the lane and it is detected
...
def evaluate_results_task2(predictions_path, ground_truth_path, verbose=0):
    ...
    for i in range(1, 16):
        ...
        try:
            ...
            gt_bboxes = gt[1:]
            num_frames = gt[0][0]
            percentage = compute_percentage_tracking(
                gt_bboxes, predicted_bboxes, num_frames, verbose
            )
            ...
            if verbose:
                ...
                print(
                    "Task 2 - Tracking the vehicle: for test example number ",
                    str(i),
                    " the prediction is :",
                    (1 - correct_tracked_video) * "in" + "correct",
                    "\n",
                )

            total_correct_tracked_videos = (
                total_correct_tracked_videos + correct_tracked_video
            )
...
def evaluate_results_task3(predictions_path, ground_truth_path, verbose=0):
    ...
    for i in range(1, 16):
        ...
        if verbose:
            print(
                "Task 3 - predicting trajectories: for test example number ",
                str(i),
                " the prediction is :",
                (1 - correct_prediction) * "in" + "correct" + "\n",
            )
...
predictions_path_root = "E:\AI_engineer_Project\CVs\Visual-Traffic-Monitoring-Computer-Vision-Challenge-main\submit_Task123\Dinh_Duc_Van_AIHN04\submit_train"
ground_truth_path_root = "E:\AI_engineer_Project\CVs\Visual-Traffic-Monitoring-Computer-Vision-Challenge-main\model\train"

# task1
verbose = 1
predictions_path = predictions_path_root + "Task1/"
ground_truth_path = ground_truth_path_root + "Task1/ground-truth/"
total_correct_queries, points_task1 = evaluate_results_task1(
    predictions_path, ground_truth_path, verbose
)
...
total_correct_tracked_videos_task2, points_task2 = evaluate_results_task2(
    predictions_path, ground_truth_path, verbose
)
...
total_correct_number_pins, points_task3 = evaluate_results_task3(
    predictions_path, ground_truth_path, verbose
)
